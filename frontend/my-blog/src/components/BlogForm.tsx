...
import { createBlog, fetchImagePreview, imageGeneration, variationGeneration } from "../services/blogService.js";
...
import BlobsBackground from "./BlobsBackground.jsx";
import { v4 as uuidv4 } from 'uuid'
import { TiptapEditor } from "./TiptapEditor.js"; 
import { uploadBlogImage } from "../services/blogService.js"

interface Variation {
    blog_version_no: number;
    blog_content: string;
}
...
const BlogForm = () => {
    ...
    const [selectedVariationNumber, setSelectedVariationNumber] = useState<number | null>(null);
    const [selectedVariationContent, setSelectedVariationContent] = useState<string | null>(null);
    ...
    const handleArtStyleSelect = (value: string) => {
        setSelectedArtStyle(value);
        console.log('Selected:', value);
    }

    const handleVariationSelect = (number: number, content: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try{
            ...
            let newBlog: { title: string; body: string; image_url?: string };
            if(file || imgSrc2){
    ...
    const handleInlineImageUpload = async (file: File): Promise<string> => {
        // you can show a toast or spinner if you want
    ...
    const handleBlogVariations = () => {
        ...
        async function generateVariations(blogInfo: {title: string, tone: string, content: string}){
            try{
    ...
    const handleGenerateImage = () => {
        ...
        async function generateImage(blogInfo: {title: string, tone: string, art_style: string}){
            try{
        ...
        <div className="container flex flex-col justify-center mx-auto pt-10 pb-24 gap-10">
            ...
            onSubmit={handleSubmit}>
                ...
                <div className="flex flex-col gap-2 px-4 ">
                            ...
                            <Menu as='div' className="relative inline-block">
                                ...
                                <MenuItems 
                                    ...
                                    {tones.map((name, index) => (
                                        <MenuItem key={index}>
                                            <div className="block px-4 py-2 text-sm text-gray-700 hover:cursor-pointer hover:bg-gray-100 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden">
                                            <button
                                                type="button"
                    ...
                    <TiptapEditor value={body} onChange={setBody} onUploadImage={handleInlineImageUpload}/>
                ...
                {variationsLoading ? (
                    ...
                    variations.length > 0 && (
                        <div className='flex gap-3 px-4 h-48'>
                            {variations.map((item) => (
                                <div key={item.blog_version_no} onClick={() => {handleVariationSelect(item.blog_version_no, item.blog_content); setIsBlogModalOpen(true)}} className="w-full hover:scale-105 duration-200 ease-in-out py-2 bg-purple-100/50 rounded-md hover:cursor-pointer border-2 border-gray-300 px-2">
                ...
                <div className="flex flex-col gap-2 px-4">
                    ...
                    <div className="flex justify-between">
                        <div className="w-1/2 flex justify-center">
                            ...
                            <label htmlFor="imageUpload" className="cursor-pointer px-3 py-2 text-white bg-indigo-400 text-center rounded-md">
                                Choose a Photo
                            </label>
                        </div>
